...
for t, v in kf.split(x_train):
    knn.fit(x_train[t], y_train[t])  # fitting
    val_score = knn.score(x_train[v], y_train[v])
    kf_score.append(val_score)

print('train time: {:.5f}s'.format(time.time() - tt))

# 测试结果
tt = time.time()
accuracy_score = knn.score(x_test, y_test)
print('test time: {:.5f}s'.format(time.time() - tt))
